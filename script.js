...
// ===========================================
// DATA FETCHING (LEADERBOARD & GALLERY)
// ===========================================
    ...
    try {
        ...
        tableBody.innerHTML = data.map(item => `
            <tr>
                ...
                <td><span class="participant-name">${item.team}</span></td>
                <td><span class="score-value">${item.score}</span></td>
                <td><span class="meme-count">${item.memes}</span></td>
            </tr>
        `).join('');
    } catch (error) {
...
async function loadGalleryData() {
    const galleryGrid = document.getElementById('meme-gallery');
    ...
    try {
        ...
        if (images.length === 0) {
            galleryGrid.innerHTML = '<p class="section-desc centered">No memes published yet. Check back soon!</p>';
            return;
        }

        galleryGrid.innerHTML = images.map(img => `
            <div class="gallery-item" data-aos="zoom-in">
                <img src="${img}" alt="Event Meme" loading="lazy">
                <div class="gallery-overlay">
                    <span class="view-btn">View Masterpiece</span>
                </div>
            </div>
        `).join('');
